...
artist_genre_df.to_csv("./output/artist_genre_info.csv", index=False)
...
artist_df.to_csv("./output/artist_info.csv", index=False)
...
song_df.to_csv("./output/song_info.csv", index=False)

album_counts = pd.read_csv("./output/song_info.csv", index_col=0)
album_counts = album_counts.drop('popularity', axis=1)
album_counts = album_counts.groupby("album", sort=False).agg(lambda x: ', '.join(x.unique()))

album_counts["count"] = song_df.album.value_counts()
album_counts.to_csv("./output/album_info.csv", index=False)
